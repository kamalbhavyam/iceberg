...
public class TestMetricsWithZorderSerialization {
  ...
  private static Metrics generateMetrics() {
    ...
    Integer zorderLower = 18;
    Integer zorderUpper = 26;

    List<Integer> zorderColumns = new ArrayList<Integer>();
    zorderColumns.add(1);
    zorderColumns.add(2);

    return new Metrics(0L, longMap1, longMap2, longMap3, byteMap1, byteMap2, zorderLower, zorderUpper, zorderColumns);
  }
  ...
  private static Metrics generateMetricsWithNulls() {
    ...
    Integer zorderUpper = 22;

    List<Integer> zorderColumns = new ArrayList<>();
    zorderColumns.add(null);
    zorderColumns.add(2);

    return new Metrics(null, null, longMap, longMap, null, byteMap, null, zorderUpper, zorderColumns);
  }
  ...
  private static void assertEquals(Metrics expected, Metrics actual) {
    ...
    Assert.assertEquals(expected.zorderLowerBound(), actual.zorderLowerBound());
    Assert.assertEquals(expected.zorderUpperBound(), actual.zorderUpperBound());
    Assert.assertEquals(expected.zorderColumns(), actual.zorderColumns());
  }
