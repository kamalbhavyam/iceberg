 ...
 */
private[sql] object IcebergLogicalExpressions {
  ...
  def zorder(references: Array[NamedReference]): ZorderTransform = ZorderTransform(references)
...
private[sql] final case class ZorderTransform(columns: Seq[NamedReference]) extends Transform {
  ...
  override def describe(): String = s"zorder(${arguments.map(_.describe).mkString(", ")})"
...
private[sql] object ZorderTransform {
  def unapply(transform: Transform): Option[(NamedReference)] = transform match {
    case NamedTransform("zorder", Seq(
    Ref(seq: Seq[String]))) =>
      Some(FieldReference(seq))
    case _ =>
      None
  }
}
